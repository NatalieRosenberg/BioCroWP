...
assume that the source files are in a directory named  'BioCroWP'.

- From the command line
```
cd path_to_unzipped_directory
R CMD INSTALL BioCroWP
```

- Or from within R
```
setwd('path_to_unzipped_directory')
install.packages('BioCroWP', repos=NULL, type='SOURCE')
```
...
library(BioCro)
library(BioCroWP)
get_all_modules('BioCroWP')
module_info('BioCroWP:example_module')
evaluate_module('BioCroWP:example_module', list(A = 1, B = 2))
```
...
The `BioCroWP` R package is licensed under the MIT license, while the BioCro C++
